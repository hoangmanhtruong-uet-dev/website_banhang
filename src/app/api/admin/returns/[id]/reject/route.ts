...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  return createHandler(async (request: NextRequest) => {
    ...
    return OrderStateService.transition({ orderId: record.orderId, targetStatus: ORDER_STATUS.RETURN_REJECTED, actor: { type: 'ADMIN', userId: actor.userId }, reason: 'Return rejected', idempotencyKey: key });
  })(req);
}
